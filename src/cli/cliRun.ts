...
import type { CodeTreeOutputStyle } from "../config/configSchema.js";
...
export const run = async () => {
  try {
    program
      .description(
        "CodeTree - Pack your repository into a single AI-friendly file"
      )
      .arguments("[directory]")
      ...
      .option(
        "--style <type>",
        "specify the output style (plain, xml, markdown)"
      )
      .option("--verbose", "enable verbose logging for detailed output")
      .option("--init", "initialize a new codetree.config.json file")
      .option(
        "--global",
        "use global configuration (only applicable with --init)"
      )
      .option("--remote <url>", "process a remote Git repository")
      .action((directory = ".", options: CliOptions = {}) =>
        executeAction(directory, process.cwd(), options)
      );
